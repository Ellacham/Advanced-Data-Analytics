...
library(caret)
library(leaps)
data <- read.csv("C:\\Users\\amuku\\Downloads\\updated_cleaned_dataset_balanced (1).csv")
...
predictors <- c('Kidhome', 'Teenhome', 'LastPurchase', 'Fruits', 'Sweets', 'Gold', 'Meats', 'Fish', 'DiscountPurchases', 
                ...
                'Recency_Intervals_More than 90 days', 'hcWardD4clusters', 'Kmeans3clusters', 'hcWardD7clusters', 'Kmeans4clusters')
...
xvars <- data[, c("Teenhome",  "Meats", "LastPurchase", "DiscountPurchases" , "CatalogPurchases", "WebVisitsMonth" ,"WebPurchases" ,
                  "StorePurchases", "Gold", "AcceptedCmp3" , "AcceptedCmp5" ,"AcceptedCmp1", "AcceptedCmp2", "Education_non_graduate",
                  "Marital_Status_Married", "Marital_Status_Together","Kmeans3clusters", "Response")]
str(xvars)
data <- xvars
...
pred <- prediction(logit.reg.pred[,2], holdout$Response)
...
sorted_actual <- as.numeric(holdout$Response[sorted_indices])
